...
```text
Java Test Framework Container → Streaming Lab Orchestration API → Browser Automation → Marker Decoding → Assertions → Artifacts
```
...
* Streaming Lab Orchestration API integration;
...
3. Call the Streaming Lab Orchestration API to prepare the run.
4. Start network capture if enabled.
5. Start stream through the Streaming Lab Orchestration API.
...
The framework should use the Streaming Lab Orchestration API to control lifecycle operations.

The Streaming Lab Orchestration API should wrap infrastructure actions such as:
...
| Area                     | Streaming Lab Orchestration API | Test Framework |
| ------------------------ | ---------------- | -------------- |
...
* API calls to Streaming Lab Orchestration API;
...
## Cloud-Native Test Framework Execution

The Java test framework should be designed as a **cloud-native test runner**, not as a long-running application service.

The framework should run as an ephemeral containerized job.
...
1. start the test-runner container;
2. load configuration from environment variables or YAML;
3. call the Streaming Lab Orchestration API;
4. open browser display clients using Playwright;
5. capture rendered output;
6. decode markers;
7. run deterministic assertions;
8. write artifacts to a mounted volume;
9. exit with status code `0` for pass or non-zero for failure.

The framework should be able to run:

* locally through Maven;
* locally through Docker Compose;
* in GitHub Actions;
* later as a Kubernetes Job.

---

## Playwright-Ready Test Runner Image

The Java test framework will use a Playwright-ready base image so that browser dependencies are already available inside the container.

The initial Docker image should use:

```dockerfile
FROM mcr.microsoft.com/playwright/java:v1.44.0-jammy
```

The Playwright image version should match the Playwright Java dependency version in `pom.xml`.
...
If the Playwright dependency is upgraded, the Docker base image should be upgraded at the same time.

Recommended Dockerfile:

```dockerfile
# test-frameworks/java/Dockerfile
...
ENV TEST_PROFILE=e2e

CMD ["mvn", "verify", "-Pe2e"]
```

This image should be treated as a test runner. It starts, runs the selected test suite, writes artifacts, and exits.

---

## Docker Compose Test Runner Service

The Java test framework should be runnable as a Docker Compose service.

Recommended service name:

```text
streamsync-test-runner
```
...
services:
  ...
  streamsync-test-runner:
    ...
    depends_on:
      ...
      - display-client
    environment:
      ORCHESTRATOR_BASE_URL: http://stream-orchestrator:8080
      DISPLAY_BASE_URL: http://display-client:3000
...
Inside Docker Compose, the framework should use service names:

```text
http://stream-orchestrator:8080
http://display-client:3000
http://mediamtx:8888
```
...
The same container image should support different execution modes.

Framework unit tests:

```bash
docker compose run --rm streamsync-test-runner mvn test
```

Framework verification tests:

```bash
docker compose run --rm streamsync-test-runner mvn clean verify -Pframework-tests
```

Streaming E2E validation:

```bash
docker compose run --rm streamsync-test-runner mvn clean verify -Pe2e
```

The test runner should write all generated artifacts to:

```text
/app/artifacts
```

Docker Compose should mount this to the host:
...
This ensures screenshots, reports, marker samples, logs, latency data, offsets, and packet-capture references remain available after the container exits.

---

## Exit Code Contract

The test runner container should provide a clear exit code contract.

| Exit Code | Meaning |
|---:|---|
| `0` | Selected test suite passed. |
| Non-zero | Selected test suite failed or execution could not complete. |

CI/CD should use the container exit code as the workflow result while still uploading artifacts with `if: always()`.

This allows failed E2E runs to preserve evidence while still failing the pipeline.
...
| `TestControlClient`      | Calls Streaming Lab Orchestration API endpoints.                                        |
...
* Streaming Lab Orchestration API logs where available;
* stack traces;
...
* Streaming Lab Orchestration API client behavior using mocks or test doubles;
...
* framework can call Streaming Lab Orchestration API;
...
* browser automation;
* streaming behavior;
* Streaming Lab Orchestration API behavior;
...
### Recommended Docker Compose Execution

The streaming E2E workflow should run the Java framework through the Playwright-ready test-runner container.
...
The test-runner container should use the Playwright-ready base image and should communicate with services through Docker service names:
...
The workflow should upload the `artifacts/` folder with `if: always()` so failed streaming runs still preserve screenshots, logs, decoded markers, latency data, packet-capture references, and reports.
...
18. Add Streaming Lab Orchestration API client.
19. Add component tests with mock API.
20. Add Playwright browser wrapper.
21. Add Playwright-ready Dockerfile for the test-runner container.
22. Add screenshot or frame capture.
23. Add report model.
24. Add HTML report generator.
25. Add unit/component tests for report generation.
26. Add network capture artifact verifier.
27. Add OpenSpec coverage mapping.
28. Add HLS end-to-end test.
29. Add latency mode end-to-end test.
30. Add network capture verification.
31. Add CI/CD workflow for framework verification tests.
32. Add CI/CD workflow for video streaming E2E validation.
33. Add CI artifact upload for both workflows.
...
| Tests become infrastructure-heavy           | Test methods become hard to read.           | Use Streaming Lab Orchestration API and domain abstractions.           |
| Browser automation becomes flaky            | Results may be unstable.                    | Separate readiness checks from marker-based validation. |
| Playwright image and dependency versions drift | Browser execution may fail in Docker. | Keep the Playwright-ready base image version aligned with the Playwright Java dependency. |
...
The framework should be Java-first, deterministic, modular, artifact-first, configuration-driven, and cloud-native as an ephemeral Playwright-ready test-runner container. It should validate streaming behavior through decoded markers, timestamps, frame numbers, browser captures, and structured artifacts.
