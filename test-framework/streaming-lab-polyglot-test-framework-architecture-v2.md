...
One central test orchestrator service:
  streaming-lab-orchestration-api
Three independent framework implementations:
...
streaming-lab-orchestration-api is the test orchestrator service.
Each language implementation drives the same behavior through that orchestrator.
Each implementation must satisfy the same config, evidence, artifact, and reporting contract.
```

Critical component correction:

```text
streaming-lab-orchestration-api is not just another API under test.
It is the central test orchestrator module for the Streaming Lab system.
```

It produces the canonical `testRunId`, owns the test-run lifecycle, exposes the stream lifecycle actions, and becomes the integration boundary that Java, Node/TypeScript, and Python must all use consistently.
...
streaming-lab-orchestration-api
  -> Spring Boot API/module that acts as the test orchestrator service
  -> creates test runs
  -> generates the canonical testRunId
  -> manages prepare/stream/stop/fail lifecycle transitions
  -> provides the integration boundary for all framework implementations
...
### 2.1 Central role of `streaming-lab-orchestration-api`

`streaming-lab-orchestration-api` must be treated as a first-class architecture component.

Its role is:

```text
streaming-lab-orchestration-api
  = test orchestrator service provider
  = source of testRunId
  = owner of test-run lifecycle state
  = system entry point for stream lifecycle control
  = common integration boundary for Java, Node, and Python frameworks
```

The polyglot frameworks do not independently invent test-run lifecycle state. They must call the orchestrator and use the state returned by the orchestrator.

This matters because the project needs one consistent source of truth for:
...
### 2.2 Relationship between orchestrator and framework implementations

The relationship should be:

```text
Shared BDD feature files
  -> define expected behavior

Java / Node / Python step definitions
  -> implement the steps in each language

OrchestrationApiClient in each language
  -> calls streaming-lab-orchestration-api

streaming-lab-orchestration-api
  -> creates and controls the test run
  -> returns testRunId and lifecycle status

Evidence writers in each implementation
  -> write artifacts using the returned testRunId
```

So the implementations are independent, but they are not independent from the system contract.

They all integrate through equivalent clients:
...
Each client may use language-specific libraries, but the behavior must remain equivalent.

### 2.3 Boundary rule

The orchestrator owns test-run state.

The framework implementations own test execution and evidence.

```text
streaming-lab-orchestration-api owns:
...
Java/Node/Python frameworks own:
  loading framework config
  executing BDD steps
  calling the orchestrator
  opening the Display Client
  validating playback behavior
  writing evidence
  publishing artifacts/results
```

This separation prevents each implementation from becoming its own mini-orchestrator.
...
```text
streaming-lab-orchestration-api/
  Spring Boot test orchestrator service
  generates testRunId
  owns test-run lifecycle
  exposes lifecycle and stream-control endpoints
...
test-framework-contract/
  ...
  shared orchestrator API integration expectations
...
```text
BDD scenarios
Orchestrator API integration rules
Config fields
Artifact layout
Evidence schemas
Required files
...
orchestrationApi:
  # This points to streaming-lab-orchestration-api.
...
- Where is `streaming-lab-orchestration-api`?
- Which orchestrator profile/environment is active?
...
```text
Only streaming-lab-orchestration-api generates the canonical testRunId.
Framework implementations consume and propagate it.
```
...
### 23.1 `streaming-lab-orchestration-api` as test orchestrator

The orchestrator module is responsible for creating and managing the test run used by each framework implementation.
...
Primary responsibility:
  expose a stable API that allows any test-framework implementation to create, control, observe, and finalize a streaming test run
```

The orchestrator is the place where the test run becomes real.

A framework implementation should not create its own synthetic run ID, fake lifecycle, or local state machine as a substitute for the orchestrator.

### 23.2 Required integration flow

Each implementation must integrate with the orchestrator using the same logical flow:

```text
1. Load test-framework config.
2. Resolve orchestrationApi.baseUrl.
3. Call POST /api/test-runs.
4. Extract testRunId from the response.
5. Store testRunId in scenario context.
6. Create artifact directory using testRunId.
7. Call POST /api/test-runs/{testRunId}/prepare.
8. Verify PREPARING or expected prepare response.
9. Call POST /api/test-runs/{testRunId}/stream.
10. Verify STREAMING or expected stream response.
11. Build Display Client URL using config + stream metadata.
12. Validate browser playback.
13. Write playback evidence using testRunId.
14. Call POST /api/test-runs/{testRunId}/stop.
15. Verify STOPPED through response or GET /api/test-runs/{testRunId}.
16. Attach/publish all artifacts using testRunId.
```

Failure handling rule:

```text
If an implementation fails after creating a testRunId, it should attempt to mark the test run as FAILED or STOPPED through streaming-lab-orchestration-api before exiting, when safe to do so.
```

This keeps the orchestrator state aligned with the framework outcome.

### 23.3 Common client contract

Each language should implement an equivalent client.
...
OrchestrationApiClient
  ...
  prepareTestRun(testRunId) -> TestRun
  startStream(testRunId) -> TestRun
  stopTestRun(testRunId) -> TestRun
  failTestRun(testRunId, reason) -> TestRun
  getTestRun(testRunId) -> TestRun
```

The class names can differ by language, but the methods and behavior should be equivalent.

Java:

```text
test-framework-jvm/src/test/java/.../client/OrchestrationApiClient.java
```

Node/TypeScript:

```text
test-framework-node/src/client/orchestrationApiClient.ts
```

Python:

```text
test-framework-python/src/client/orchestration_api_client.py
```

### 23.4 Scenario context contract

Each implementation needs a scenario context object that stores values returned by the orchestrator and used by later BDD steps.

Required scenario context fields:
...
The context is language-specific, but the logical data must be the same.

Example responsibility flow:

```text
Given a new test run exists
  -> calls orchestrationApiClient.createTestRun()
  -> stores testRunId
  -> creates artifact directory

When the stream is started
  -> calls orchestrationApiClient.startStream(testRunId)
  -> stores STREAMING status
  -> resolves stream URL

Then all required artifacts should contain the test run id
  -> verifies testRunId exists in JSON evidence, logs, filenames/metadata, and ReportPortal attributes
```

### 23.5 Stream metadata integration

The cleanest future design is for `streaming-lab-orchestration-api` to return enough metadata for the test framework to know what stream/display URL to test.

Potential response fields:

```json
{
  "id": "8f6b2a4e-0000-4000-9000-000000000000",
  "status": "STREAMING",
  "stream": {
    "streamId": "camera_sync_test",
...
```text
The framework can build the HLS URL from config:
  streaming.hlsBaseUrl + streaming.streamId

Later improvement:
  orchestration-api returns stream metadata directly.
```

This is an important evolution point. The more the orchestrator owns stream metadata, the less each framework needs environment-specific URL logic.

### 23.6 What not to duplicate in each framework

Do not duplicate orchestration rules in Java, Node, and Python.

Avoid:

```text
Each framework has its own hardcoded lifecycle rules.
Each framework independently decides if CREATED can go to STREAMING.
Each framework generates its own run ID.
Each framework has separate stream naming assumptions.
```

Instead:

```text
streaming-lab-orchestration-api enforces lifecycle rules.
Frameworks call the API and validate the API response.
Frameworks record evidence of the observed behavior.
```

The frameworks can have enum/model classes for response parsing, but they should not become the source of lifecycle truth.
...
| Calls `streaming-lab-orchestration-api` through an equivalent client | Required | Required | Required |
| Consumes `testRunId` generated by `streaming-lab-orchestration-api` | Required | Required | Required |
...
docs/test-framework/orchestrator-integration-contract.md
...
- streaming-lab-orchestration-api is the central test orchestrator service provider.
- It generates the canonical testRunId, owns the test-run lifecycle, exposes prepare/stream/stop/fail endpoints, and is the integration boundary for all framework implementations.
...
- All implementations must satisfy the same orchestrator integration contract, config contract, evidence schema, artifact layout, testRunId correlation rules, and ReportPortal tag rules.
- ReportPortal is the selected dashboard tool.
